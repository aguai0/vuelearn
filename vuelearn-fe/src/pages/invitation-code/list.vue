  ...
  <el-row>
    <el-col :span="24">
      <el-form :inline="true" :model="query">
        ...
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" @click="currentDetail">查看选中项</el-button>
        </el-form-item>
      </el-form>
    </el-col>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-radio v-model="currentRow" :label="scope.row.invitationCode"><i></i></el-radio>
          </template>
        </el-table-column>
        <el-table-column
          prop="invitationCode"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      choose (item) {
        this.currentRow = item.invitationCode
        /**
         * 如本页面可能为公共页面，会被其他页面唤起打开，则需在选中列表项的处理函数的最后加入pageOver函数，将会自动调用唤起页的回调函数
         */
        this.pageOver(item)
      },
      currentDetail () {
        this.$message(`当前选中项邀请码为${this.currentRow}`)
      },
      typeFormat (row) {
        const constant = {
          1: '兼职司机',
